...
class BlastLine(object):
    ...
    def __init__(self, sline, mode="1"*12):
        args = sline.split("\t")
        assert 0 < int(mode, 2) <= 4095, "mode can only contain 0 or 1"
        mode = list(mode)
        self.query = args[0] if mode[0]=="1" else None
        self.subject = args[1] if mode[1]=="1" else None
        self.pctid = float(args[2]) if mode[2]=="1" else -1
        self.hitlen = int(args[3]) if mode[3]=="1" else -1
        self.nmismatch = int(args[4]) if mode[4]=="1" else -1
        self.ngaps = int(args[5]) if mode[5]=="1" else -1
        self.qstart = int(args[6]) if mode[6]=="1" else -1
        self.qstop = int(args[7]) if mode[7]=="1" else -1
        self.sstart = int(args[8]) if mode[8]=="1" else -1
        self.sstop = int(args[9]) if mode[9]=="1" else -1
        self.evalue = float(args[10]) if mode[10]=="1" else -1
        self.score = float(args[11]) if mode[11]=="1" else -1

        if self.sstart == -1 or self.sstop == -1:
            self.orientation = '.'
        elif self.sstart > self.sstop:
...
class BlastSlow (LineFile):
    ...
    """
    def __init__(self, filename, sorted=False, blmode="1"*12):
        super(BlastSlow, self).__init__(filename)
        fp = must_open(filename)
        for row in fp:
            self.append(BlastLine(row, blmode))
        if not sorted:
...
class Blast (LineFile):
    ...
    def __init__(self, filename):
        super(Blast, self).__init__(filename, blmode="1"*12)
        self.fp = must_open(filename)
        self.blmode = blmode

    def iter_line(self):
        for row in self.fp:
            yield BlastLine(row, self.blmode)

    def iter_hits(self):
        for query, blines in groupby(self.fp,
                key=lambda x: BlastLine(x, self.blmode).query):
            blines = [BlastLine(x, self.blmode) for x in blines]
...
def annotation(args):
    ...
    blast = Blast(blastfile, blmode="110000000000")
