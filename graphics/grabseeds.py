...
def calibrate(args):
    """
    %prog calibrate batch-output.tsv boxsize

    Calibrate pixel-inch ratio and color adjustment.
    - `batch-output.tsv` is the file generated by batchseeds(). `
    - `photo-ID` is the photo that contains the colorchecker image
    ...
    p.add_option("--name", default="calibration",
                 help="Accession name for calibration entries")
    opts, args = p.parse_args(args)

    if len(args) != 2:
        sys.exit(not p.print_help())

    tsvfile, boxsize = args
    name = opts.name
    ...
    xs = [t.x for t in seeds]
    ys = [t.y for t in seeds]
    idx_xs = get_kmeans(xs, 6)
    idx_ys = get_kmeans(ys, 4)
    for xi, yi, t in zip(idx_xs, idx_ys, seeds):
        t.rank = (yi, xi)
