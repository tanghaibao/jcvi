...
def pasa(args):
    ...
    if need_update((fastafile, gffile), (transcodergff, transcodergenomegff)):
        ...
        cmd += " --pasa_transcripts_fasta {0} --pasa_transcripts_gff3 {1}".format(
            fastafile, gffile
        )
        sh(cmd)
...
def augustus(args):
    ...
    sh(
        "{0}/scripts/gff2gbSmallDNA.pl {1} {2} 1000 raw.gb".format(
    ...
    if opts.autotrain:
        sh("rm -rf {0}".format(target))
        sh(
            "{0}/scripts/autoAugTrain.pl --trainingset=training.gb --species={1}".format(
                mhome, species
            )
        )
