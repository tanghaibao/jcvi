...
def mainArgs():
    ...
    description =   'Convert whole-genome alignments to block & bed format for ribbon.',
    prog        =   'jvci.apps.wga2ribbon')
    #parser.add_argument('-a', '--alignments', nargs='+', required=True, help='Space delimited list of WGA files in same order as layout file.')
    parser.add_argument('-a', '--alignments', action='append', help='Set arg for each alignment file to be imported. Has format "[Source chrm Idx],[Hit chrm Idx],[filename]" i.e. For file with alignmnets between the first and second tracks in ribbon.layout: -a "0,1,A2B_Alignment.tab"', required=True)
    parser.add_argument('-l', '--layout', type=str, required=True, help='Ribbon plot layout file.')
    parser.add_argument('--minID', type=float, default=50, help='Extract alignments with identity => than minID')
    parser.add_argument('--outBed', type=str, default='ribbons.bed', help='Write alignment block coordinates to this bedfile')
    parser.add_argument('--outBlocks', type=str, default='ribbons.block', help='Write alignment block names to this file.')
...
def main():
	...
	for coords in args.alignments:
		...
		for x in sorted(hits[plotlines[selfIdx].ChrmName][plotlines[hitIdx].ChrmName], key = lambda x: (int(x.t_start), int(x.t_end))):
			tName = str(plotlines[selfIdx].ChrmName)
			qName = str(plotlines[hitIdx].ChrmName)
			tUID = '_'.join([str(selfIdx),str(hitIdx),str(selfIdx),'ID'+str(x.UID),tName,str(x.t_start),str(x.t_end)])
			qUID = '_'.join([str(selfIdx),str(hitIdx),str(hitIdx),'ID'+str(x.UID),qName,str(x.q_start),str(x.q_end)])
			bedlines[selfIdx].append(tuple( [tName,str(x.t_start),str(x.t_end),tUID,str(x.idPct),str(x.t_strand)] ))
			bedlines[hitIdx].append(tuple( [qName,str(x.q_start),str(x.q_end),qUID,str(x.idPct),str(x.q_strand)] ))
