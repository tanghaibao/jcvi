...
def mainArgs():
    ...
    parser.add_argument('-a', '--alignments', action='append', required=True,
    ...
    parser.add_argument('-l', '--layout', type=str, required=True,
                        help='Ribbon plot layout file.')
    parser.add_argument('--minID', type=float, default=50,
                        help='Extract alignments with identity => than minID')
    parser.add_argument('--outBed', type=str, default='ribbons.bed',
                        help='Write alignment block coords to this bedfile')
    parser.add_argument('--outBlocks', type=str, default='ribbons.block',
...
def readLASTZ(infile, minID=50):
    ...
    hitTup = namedtuple('Elem', ['t_start', 't_end', 't_strand', 'q_start',
                                 'q_end', 'q_strand', 'idPct', 'UID'])
    counter = 0
    # Read in split rows
    for row in content:
    ...
    for row in content:
        ...
        if rowID >= minID:
            ...
            if q_name not in list(hitsDict[t_name].keys()):
                hitsDict[t_name][q_name] = list()
            # Write record to target:query list as named tuple
            hitsDict[t_name][q_name].append(hitTup(t_start,
                                                   t_end,
                                                   t_strand,
                                                   q_start,
                                                   q_end,
...
def main():
    ...
    for coords in args.alignments:
        ...
        for x in sorted(hits[plotlines[selfIdx].ChrmName][plotlines[hitIdx].ChrmName], key = lambda x: (int(x.t_start), int(x.t_end))):
            ...
            qUID = '_'.join([str(selfIdx),
                             ...
                             str(x.q_start),
                             str(x.q_end)])
            bedlines[selfIdx].append(tuple([tName,
                                            str(x.t_start),
                                            str(x.t_end),
                                            tUID,
                                            str(x.idPct),
                                            str(x.t_strand)]))
            bedlines[hitIdx].append(tuple([qName,
                                           str(x.q_start),
                                           str(x.q_end),
                                           qUID,
                                           str(x.idPct),
    ...
    with open(args.outBed, 'w') as f:
        for layoutIdx in list(bedlines.keys()):
            for line in sorted(bedlines[layoutIdx],
                               key=lambda x:
                                   (x[0], int(x[1]), int(x[2]), x[3])):
                f.write('\t'.join(line) + '\n')
