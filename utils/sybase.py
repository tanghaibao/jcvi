...
def get_profile(sqshrc="~/.sqshrc"):
    ...
    for row in open(sqshrc):
        row = row.strip()
        if not row.startswith("\set"): continue
        if "username" in row: username = _(row)
        elif "password" in row: password = _(row)
        elif "hostname" in row: hostname = _(row)
...
if __name__ == '__main__':
    print get_profile()
    print connect("track", "select bac_name from bac where bac_name like 'mth2%'")
